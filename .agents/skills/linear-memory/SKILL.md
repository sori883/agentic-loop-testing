---
name: linear-memory
description: Linear を agent loop の永続 memory backend と状態管理 backend として使うための Skill。Codex または subagent が Linear の work item を読む、実行ログを記録する、status/label による task state を更新する、判断を保存する、人間への確認事項を Linear にエスカレーションする必要があるときに使う。chat context に依存せず、Linear を source of truth として扱う。
---
...
- `Todo`: 実行可能。Goal、Acceptance Criteria、対象範囲が十分に明確。
- `In Progress`: agent または人間が現在作業中。
- `In Review`: phase-reviewer、verifier、または人間の確認待ち。
- `Done`: 検証と必要な review gate が通り、完了として扱える。
...
- `needs-triage`: まだ Goal / Acceptance Criteria / scope が足りない。
- `ready-for-agent`: agent が実行してよい。
- `agent-active`: agent が作業中。
...
- `Todo -> In Progress`: agent が claim し、作業を開始した。
- `In Progress -> In Review`: design/test/implementation/refactor/verification のいずれかで review gate 待ちになった。
- `In Review -> In Progress`: reviewer が `REJECT` し、修正が必要になった。
- `In Review -> Done`: 最終 verification と review gate が `APPROVE` になった。
...
全 agent が自由に status を動かしてはいけない。役割ごとに許可範囲を分ける。

- Triage: `Backlog -> Todo`、`needs-triage` / `ready-for-agent` の更新を行ってよい。
- Implementer: `Todo -> In Progress`、phase label、`agent-active`、GitHub link の記録を行ってよい。
- Phase Reviewer: `In Progress -> In Review` の確認結果を Comment に記録し、`needs-review` / `verification-failed` を提案してよい。status 更新は project policy が許可する場合だけ行う。
- Verifier: 最終 `APPROVE` の場合に `In Review -> Done` を行ってよい。`REJECT` の場合は `In Progress` へ戻してよい。
- Human / Controller: すべての state transition を行ってよい。

project policy が未定義の場合の安全な default:

- agent は Comment と label を更新する。
...
この Skill の State Machine を default として使う。Linear Document または user request に project 固有の状態遷移ルールがある場合は、それを優先する。

安全な default:

- Agents は Comments を追記してよい。
- Agents は自分の作業を説明する links を追加してよい。
- project policy または State Update Permissions がその role に許可していない限り、Agents は status 移動、issue close、owner 変更をしない。
