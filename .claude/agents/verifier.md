...
Linear や GitHub を変更してもいけません。各 phase の `$cross-review-gates`（phase-reviewer 担当）の結果は、
`linear-memory` skill で証跡として読み取るだけにし、自分でゲートを起動・別エージェントの spawn はしません。
...
（証跡: 成功したテスト / 検証コマンドと結果、満たした受け入れ基準、GitHub / CI リンク）
