...
- controller はレビュー担当に、成果物、目的、受け入れ条件、関連差分（git diff または対象 ref）、テスト出力など、判定に必要な証拠を prompt 内で明示して渡す。Linear Comment 化前でもレビューできるよう、証跡は prompt に載せる（レビュー担当は Linear MCP を持たない場合があるため）。
...
レビュー担当（phase-reviewer 等）は read-only であり、Linear には書き込まない。各 review gate の結果は Comment 案として controller に返し、Linear が memory backend の場合は controller が `$linear-memory` を使って Comment に記録する。
