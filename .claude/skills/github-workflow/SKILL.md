...
description: GitHub をコーディングエージェントループの作業面として使うための Skill。Claude Code のサブエージェントが GitHub repository、branch、commit、pull request、review comment、CI/check failure、GitHub issue を扱う必要があるときに使う。read は全エージェント可。GitHub の状態変更（branch/commit/push、PR 作成・更新、issue/label 変更）は controllerが適用し、read-only サブエージェントは案・証跡を返す。Linear をメモリバックエンドとするプロジェクトでは、GitHub は実装成果物と検証証跡の場所として扱い、永続的な作業状態は linear-memory に記録する。
---
...
GitHub の状態変更（branch / commit / push、PR の作成・更新、issue / label / assignee 変更、CI retry）は controllerが適用する。read-only サブエージェントは branch 名案・commit message 案・PR 本文案・状態変更案・検証証跡案を返すだけで、自分で GitHub 状態を変更しない（`CLAUDE.md` の write boundary）。以降の手順はこの前提で読む。
...
- Branch: Linear の「Copy git branch name」を使える場合はそれを使う。手動で作る場合は issue ID を含める。
  - 例: `claude/ENG-123-add-login-e2e`
...
- branch 名は既存方針がなければ `claude/<linear-issue-id>-<short-topic>` を使う。Linear issue がない場合だけ `claude/<short-topic>` を使う。
