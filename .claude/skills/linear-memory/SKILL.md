---
name: linear-memory
description: Linear をエージェントループの永続メモリバックエンドと状態管理バックエンドとして使うための Skill。Claude Code のサブエージェントが Linear の work item を読む、実行ログ・status/label 更新・判断・エスカレーションを扱う必要があるときに使う。read は全エージェント可。Linear への書き込み（記録・status/label 更新・Issue 作成）は controllerが適用し、read-only サブエージェントは更新案・証跡案を返す。チャット文脈に依存せず、Linear を source of truth として扱う。
---
...
Linear への読み書きは `.mcp.json` に定義された `linear` MCP server 経由で行う。MCP ツールは必要時に discover して使う。
...
1. ユーザー依頼、automation prompt、`CLAUDE.md` の「ループポリシー」、利用可能な config から対象の Linear Issue、Project、または discovery scope を特定する。
...
Linear への読み書きは `linear` MCP ツール経由で行う。MCP ツールを `tools` に持つのは controller（および Linear MCP を明示付与されたエージェント）であり、read-only サブエージェント（triage / explorer / phase-reviewer / verifier）は通常 Linear MCP を持たない。これらのエージェントは Linear を自分で fetch せず、判断に必要な Linear 文脈（対象 Issue、受け入れ条件、関連 Comment、状態）を controller から prompt 内で受け取る前提で動く。
...
read-only サブエージェント（triage / explorer / phase-reviewer / verifier）は、本節の記録・Issue 作成・capability（skill/agent）追加を直接行わず、提案として controller に返す。Linear への記録と適用、capability ファイルの作成・更新は controllerのみが行う。
...
- 小さく局所的で検証可能な改善、skill 追加、agent 追加は、controllerが、project policy の claim、TDD、review gate、completion evidence に従って実行する。read-only サブエージェントは提案を返すのみ。
...
- 工程レビュー担当（phase-reviewer）: 設計、テスト、実装、リファクタリング、検証工程の各工程後に、担当エージェントとは別エージェントとして `APPROVE`、`REJECT`、`ESCALATE` を判定し、記録案を controller に返す（Linear への記録は controller。work item 全体の最終検証は verifier）。
- GitHub 連携（controller が `$github-workflow` を使う場合）: branch、commit、PR、CI run、review comment を Linear issue に紐づけ、URL または identifier を記録する。
